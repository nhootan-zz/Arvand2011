...
class tree {
 ...
 public:
  ...
  /// Append the node (plus its children) at other_position as last/first child
  /// of position.
  template <typename iter>
  iter append_child(iter position, iter other_position);
  template <typename iter>
  iter prepend_child(iter position, iter other_position);
  /// Append the nodes in the from-to range (plus their children) as last/first
  ...
  /// Insert node (with children) pointed to by subtree as previous sibling of
  ...
  /// Insert node (with children) pointed to by subtree as next sibling of node
  ...
  /// Replace node at 'position' with subtree starting at 'from' (do not erase
  /// subtree at 'from'); see above.
  template <typename iter>
  iter replace(iter position, const iterator_base& from);
  /// Replace string of siblings (plus their children) with copy of a new string
  /// (with children); see above
  ...
  /// Merge with other tree, creating new branches and leaves only if they are
  /// not already present.
  ...
  /// Exchange the node (plus subtree) with its sibling node (do nothing if no
  /// sibling present).
  ...
  int max_depth() const;
  /// Determine the maximal depth of the tree with top node at the given
  /// position.
  ...
  /// Determine whether node at position is in the subtrees with root in the
  /// range.
  ...
  /// Comparator class for iterators (compares pointer values; why doesn't this
  /// work automatically?)
  ...
  tree_node *head, *feet;  // head/feet are always dummy; if an iterator points
                           // to them it is invalid
 private:
...
// tree_node_allocator>::iterator& two)
// 	{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator
tree<T, tree_node_allocator>::end() const {
...
tree<T, tree_node_allocator>::begin_fixed(const iterator_base& pos,
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::fixed_depth_iterator
tree<T, tree_node_allocator>::end_fixed(const iterator_base& pos,
                                        unsigned int dp) const {
  assert(
      1 ==
  ...
  while (curdepth < dp) {  // go down one level
    while (tmp->first_child == 0) {
      tmp = tmp->next_sibling;
      if (tmp == 0) throw std::range_error("tree: end_fixed out of range");
    }
    tmp = tmp->first_child;
    ++curdepth;
  }
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator
tree<T, tree_node_allocator>::end_leaf() const {
  return leaf_iterator(feet);
}

template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator
tree<T, tree_node_allocator>::begin_leaf(const iterator_base& top) const {
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator
tree<T, tree_node_allocator>::end_leaf(const iterator_base& top) const {
...
iter tree<T, tree_node_allocator>::insert(iter position, const T& x) {
  if (position.node == 0) {
    position.node = feet;  // Backward compatibility: when calling insert on a
                           // null node, insert before the feet.
  }
  tree_node* tmp = alloc_.allocate(1, 0);
  kp::constructor(&tmp->data, x);
  tmp->first_child = 0;
...
template <class T, class tree_node_allocator>
template <class iter>
iter tree<T, tree_node_allocator>::insert_subtree(
    iter position, const iterator_base& subtree) {
  // insert dummy
...
template <class T, class tree_node_allocator>
template <class iter>
iter tree<T, tree_node_allocator>::insert_subtree_after(
    iter position, const iterator_base& subtree) {
  // insert dummy
...
// template <class T, class tree_node_allocator>
// template <class iter>
// iter tree<T, tree_node_allocator>::insert_subtree(sibling_iterator position,
// iter subtree)
// 	{
// 	// insert dummy
// 	iter it(insert(position, value_type()));
...
iter tree<T, tree_node_allocator>::replace(iter position,
  ...
  if (current_to->prev_sibling == 0) {
    if (current_to->parent != 0) current_to->parent->first_child = tmp;
  } else {
    current_to->prev_sibling->next_sibling = tmp;
  }
  tmp->prev_sibling = current_to->prev_sibling;
  if (current_to->next_sibling == 0) {
    if (current_to->parent != 0) current_to->parent->last_child = tmp;
  } else {
...
tree<T, tree_node_allocator>::replace(sibling_iterator orig_begin,
  ...
  while ((++orig_begin) != orig_end) orig_last = orig_last->next_sibling;
  tree_node* new_last = new_first;
  while ((++new_begin) != new_end) new_last = new_last->next_sibling;
...
iter tree<T, tree_node_allocator>::flatten(iter position) {
  if (position.node->first_child == 0) return position;
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator
tree<T, tree_node_allocator>::move_before(sibling_iterator target,
                                          sibling_iterator source) {
  tree_node* dst = target.node;
  tree_node* src = source.node;
...
void tree<T, tree_node_allocator>::sort(sibling_iterator from,
  ...
  typename std::multiset<tree_node*,
...
bool tree<T, tree_node_allocator>::equal(const iter& one_, const iter& two,
  ...
  while (one != two && is_valid(three)) {
    if (!fun(*one, *three)) return false;
    if (one.number_of_children() != three.number_of_children()) return false;
    ++one;
    ++three;
  }
  return true;
...
// tree_node_allocator>::find_subtree( 	sibling_iterator subfrom,
...
bool tree<T, tree_node_allocator>::is_in_subtree(
    const iterator_base& it, const iterator_base& begin,
    const iterator_base& end) const {
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator
tree<T, tree_node_allocator>::sibling(const iterator_base& it,
                                      unsigned int num) {
  tree_node* tmp;
  if (it.node->parent == 0) {
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator
tree<T, tree_node_allocator>::child(const iterator_base& it, unsigned int num) {
...
tree<T, tree_node_allocator>::iterator_base::begin() const {
  if (node->first_child == 0) return end();
...
tree<T, tree_node_allocator>::post_order_iterator::operator--() {
  ...
  if (this->skip_current_children_ || this->node->last_child == 0) {
    this->skip_current_children_ = false;
    while (this->node->prev_sibling == 0) this->node = this->node->parent;
    this->node = this->node->prev_sibling;
  } else {
...
template <class T, class tree_node_allocator>
tree<T, tree_node_allocator>::breadth_first_queued_iterator::
    breadth_first_queued_iterator()
    : iterator_base() {}

template <class T, class tree_node_allocator>
tree<T, tree_node_allocator>::breadth_first_queued_iterator::
    breadth_first_queued_iterator(tree_node* tn)
    : iterator_base(tn) {
  ...
  upper:
    do {
      if (this->node == this->top_node) {
        this->node = 0;  // FIXME: return a proper fixed_depth end iterator once
                         // implemented
        return *this;
      }
      this->node = this->node->parent;
...
void tree<T, tree_node_allocator>::sibling_iterator::set_parent_() {
  ...
  if (this->node->parent != 0) parent_ = this->node->parent;
}
