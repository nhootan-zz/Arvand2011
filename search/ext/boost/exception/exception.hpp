...
struct error_info_container {
  ...
  virtual refcount_ptr<exception_detail::error_info_container> clone()
      const = 0;
...
class exception {
 protected:
  exception() : throw_function_(0), throw_file_(0), throw_line_(-1) {}

#ifdef __HP_aCC
  // On HP aCC, this protected copy constructor prevents throwing
  // boost::exception. On all other platforms, the same effect is achieved by the
 ...
 private:
  ...
  friend char const *exception_detail::get_diagnostic_information(
  ...
  friend void exception_detail::copy_boost_exception(exception *,
                                                     exception const *);
#endif
  mutable exception_detail::refcount_ptr<exception_detail::error_info_container>
      data_;
...
struct enable_error_info_return_type {
  typedef typename enable_error_info_helper<
      T, sizeof(exception_detail::dispatch_boost_exception((T *)0))>::type type;
};
}  // namespace exception_detail
...
inline void copy_boost_exception(exception *a, exception const *b) {
  ...
  if (error_info_container *d = b->data_.get()) data = d->clone();
...
class clone_impl : public T, public clone_base {
 ...
 private:
  clone_base const *clone() const { return new clone_impl(*this); }
