...
// but only if BOOST_NO_STDC_NAMESPACE is not defined (if it is then we do the
// import into std:: ourselves).
//
...
//
// gcc-2.95.3/STLPort does not like the using declarations we use to get ADL
// with std::min/max
//
#if defined(__GNUC__) && (__GNUC__ < 3)
