...
LocalProblem *ContextEnhancedAdditiveHeuristic::get_local_problem(int var_no,
                                                                  int value) {
...
void ContextEnhancedAdditiveHeuristic::expand_node(LocalProblemNode *node) {
  ...
  if (reached_by) {
    ...
    if (parent->reached_by) node->reached_by = parent->reached_by;
  }
...
void ContextEnhancedAdditiveHeuristic::expand_transition(LocalTransition *trans,
                                                         const State &state) {
  ...
  for (; curr_precond != last_precond; ++curr_precond) {
    ...
    if (current_val == precond_value) continue;

    LocalProblem *subproblem = get_local_problem(precond_var_no, current_val);

    if (!is_local_problem_set_up(subproblem)) {
      set_up_local_problem(subproblem, get_priority(trans->source), current_val,
                           state);
    }
...
int ContextEnhancedAdditiveHeuristic::compute_costs(const State &state) {
  while (!node_queue.empty()) {
    ...
    if (get_priority(node) < curr_priority) continue;
    if (node == goal_node) return node->cost;
...
ContextEnhancedAdditiveHeuristic::~ContextEnhancedAdditiveHeuristic() {
  ...
  for (size_t i = 0; i < local_problems.size(); ++i) delete local_problems[i];
}
