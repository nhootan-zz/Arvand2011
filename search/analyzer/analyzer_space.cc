...
AnalyzerNode::AnalyzerNode(state_var_t *state_buffer_, AnalyzerNodeInfo &info_,
                           OperatorCost cost_type_)
    : state_buffer(state_buffer_), info(info_), cost_type(cost_type_) {}

State AnalyzerNode::get_state() const { return State(state_buffer); }
...
AnalyzerSpace::AnalyzerSpace(OperatorCost cost_type_) : cost_type(cost_type_) {
  nodes = new HashTable;
}

AnalyzerSpace::~AnalyzerSpace() { delete nodes; }

int AnalyzerSpace::size() const { return nodes->size(); }
