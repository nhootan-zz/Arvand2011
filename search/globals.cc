...
void save_plan(const vector<const Operator *> &plan) {
  ...
  if (plan_cost < best_cost) {
    ...
    statusfile << num_plans << " " << plan_cost << endl;
    statusfile.close();
...
bool peek_magic(istream &in, string magic) {
  ...
  for (int i = word.size() - 1; i >= 0; i--) in.putback(word[i]);
  return result;
}
...
void read_variables(istream &in) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    for (size_t i = 0; i < fact_names.size(); i++) getline(in, fact_names[i]);
...
void read_operators(istream &in) {
  int count;
  in >> count;
  for (int i = 0; i < count; i++) g_operators.push_back(Operator(in, false));
}

void read_axioms(istream &in) {
  int count;
  in >> count;
  for (int i = 0; i < count; i++) g_axioms.push_back(Operator(in, true));
