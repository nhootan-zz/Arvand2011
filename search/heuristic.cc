...
bool Heuristic::is_dead_end() const { return evaluator_value == DEAD_END; }
...
int Heuristic::get_value() const { return evaluator_value; }

void Heuristic::evaluate(int, bool) {
  return;
  // if this is called, evaluate(const State &state) or set_evaluator_value(int
  // val) should already have been called
}
...
void Heuristic::set_evaluator_value(int val) { evaluator_value = val; }
