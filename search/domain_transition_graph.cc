  ...
  for (int origin = 0; origin < nodes.size(); origin++) {
    ...
    for (int i = 0; i < trans_count; i++) {
      ...
      for (int j = 0; j < pre_post.size(); j++) {
        ...
        if (pre != -1) triggercond_pairs.push_back(make_pair(var_no, pre));
...
void DomainTransitionGraph::get_successors(int value,
...
void ValueTransition::simplify_labels(vector<ValueTransitionLabel> &label_vec) {
  ...
  for (HashMap::iterator it = label_index.begin(); it != label_index.end();
       ++it) {
    ...
    if (powerset_size <= 31) {  // HACK! Don't spend too much time here...
      for (int mask = 0; mask < powerset_size; mask++) {
        ...
        for (int i = 0; i < key.size(); i++)
          if (mask & (1 << i)) subset.push_back(key[i]);
        HashMap::iterator found = label_index.find(subset);
        if (found != label_index.end()) {
          if (old_labels[label_no].op->get_cost() >=
              old_labels[found->second].op->get_cost()) {
    ...
    if (!match) label_vec.push_back(old_labels[label_no]);
  }
}
