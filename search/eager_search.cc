...
int EagerSearch::step() {
  ...
  for (int i = 0; i < applicable_ops.size(); i++) {
    ...
    if (use_multi_path_dependence || succ_node.is_new()) {
      ...
      if (h_is_dirty && use_multi_path_dependence) succ_node.set_h_dirty();
    }
    ...
    if (succ_node.is_new()) {
      ...
      // TODO:CR - add an ID to each state, and then we can use a vector to save
      // per-state information
      int succ_h = heuristics[0]->get_heuristic();
      if (do_pathmax) {
        if ((node.get_h() - get_adjusted_cost(*op)) > succ_h) {
          // cout << "Pathmax correction: " << succ_h << " -> " << node.get_h()
          // - get_adjusted_cost(*op) << endl;
      ...
      if (reopen_closed_nodes) {
        ...
        if (succ_node.is_closed()) {
           ...
           * use a plain assert. */
          // TODO:CR - add a consistent flag to heuristics, and add an assert
          // here based on it
...
void EagerSearch::dump_search_space() { search_space.dump(); }
...
static SearchEngine *_parse_weighted_astar(OptionParser &parser) {
  ...
  if (!parser.dry_run()) {
    ...
    if (opts.get<int>("w") == 1) {
      ...
      WeightedEvaluator *w = new WeightedEvaluator(eval, opts.get<int>("w"));
    ...
    evals.push_back(sum_evals.back());
...
static Plugin<SearchEngine> _plugin_weighted_astar("eager_wastar",
