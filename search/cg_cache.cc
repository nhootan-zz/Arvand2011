...
CGCache::CGCache() {
  ...
  for (int var = 0; var < var_count; var++) {
    // This is to be on the safe side of overflows for the multiplications
    // below.
    ...
    for (int i = 0; i < succ.size(); i++) {
      ...
      if (succ[i] > var) depends_on[succ[i]].push_back(var);
    }
  }
  ...
  */
  ...
  for (int var = 0; var < var_count; var++) {
    int required_cache_size =
        g_variable_domain[var] * (g_variable_domain[var] - 1);
...
int CGCache::get_index(int var, const State &state, int from_val,
                       int to_val) const {
