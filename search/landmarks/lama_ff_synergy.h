...
class LamaFFSynergy {
  class HeuristicProxy : public Heuristic {
   protected:
    ...
    virtual void get_preferred_operators(
        std::vector<const Operator *> &result) = 0;
   ...
   public:
    ...
    virtual int compute_heuristic(const State &state) {
      if (is_first_proxy) synergy->compute_heuristics(state);
      return get_heuristic_value();
    }
  };

  class FFHeuristicProxy : public HeuristicProxy {
    virtual int get_heuristic_value() { return synergy->ff_heuristic_value; }
    virtual void get_preferred_operators(
  ...
  class LamaHeuristicProxy : public HeuristicProxy {
    virtual int get_heuristic_value() { return synergy->lama_heuristic_value; }
    virtual void get_preferred_operators(
        std::vector<const Operator *> &result) {
 ...
 public:
  ...
  Heuristic *get_ff_heuristic_proxy() { return &ff_heuristic_proxy; }

  Heuristic *get_lama_heuristic_proxy() { return &lama_heuristic_proxy; }
};
