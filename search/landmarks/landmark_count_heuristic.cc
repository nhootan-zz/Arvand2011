...
LandmarkCountHeuristic::LandmarkCountHeuristic(const Options &opts)
  ...
  if (opts.get<bool>("admissible")) {
    ...
    if (lgraph.is_using_reasonable_orderings()) {
      cerr
          << "Reasonable orderings should not be used for admissible heuristics"
          << endl;
      ::exit(2);
    }
    if (!g_axioms.empty()) {
    ...
    if (opts.get<bool>("optimal")) {
      ...
      lm_cost_assignment = new LandmarkEfficientOptimalSharedCostAssignment(
          lgraph, OperatorCost(opts.get_enum("cost_type")));
#else
      cerr << "You must build the planner with the USE_LP symbol defined."
           << endl
           << "If you already did, try \"make clean\" before rebuilding with "
              "USE_LP=1."
           << endl;
      exit(1);
#endif
...
void LandmarkCountHeuristic::set_exploration_goals(const State &state) {
  ...
  const vector<bool> &reached_lms_v =
      lm_status_manager.get_reached_landmarks(state);
...
int LandmarkCountHeuristic::get_heuristic_value(const State &state) {
  ...
  assert(h >= 0);
  if (h == 0 && g_min_action_cost > 0) assert(test_goal(state));
...
void LandmarkCountHeuristic::collect_lm_leaves(
  ...
  for (it = lgraph.get_nodes().begin(); it != lgraph.get_nodes().end(); it++) {
    ...
    if (!disjunctive_lms && node_p->disjunctive) continue;
...
bool LandmarkCountHeuristic::check_node_orders_disobeyed(
    LandmarkNode &node, const LandmarkSet &reached) const {
...
bool LandmarkCountHeuristic::generate_helpful_actions(
  ...
  for (int i = 0; i < all_operators.size(); i++) {
    ...
    for (int j = 0; j < prepost.size(); j++) {
      if (!prepost[j].does_fire(state)) continue;
  ...
  if (ha_disj.empty() && ha_simple.empty()) return false;
...
void LandmarkCountHeuristic::convert_lms(LandmarkSet &lms_set,
  ...
  for (int i = 0; i < lms_vec.size(); i++)
    if (lms_vec[i]) lms_set.insert(lgraph.get_lm_for_index(i));
}
...
static Plugin<ScalarEvaluator> _plugin("lmcount", _parse);
