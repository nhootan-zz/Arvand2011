...
LandmarkGraph *LandmarkFactory::compute_lm_graph() {
  ...
  if (lm_graph->number_of_landmarks() == 0)
    ...
    cout << "Discovered " << lm_graph->number_of_landmarks()
         ...
         << " are disjunctive and " << lm_graph->number_of_conj_landmarks()
         << " are conjunctive \n"
...
void LandmarkFactory::generate() {
  if (lm_graph->use_only_causal_landmarks()) discard_noncausal_landmarks();
  if (!lm_graph->use_disjunctive_landmarks()) discard_disjunctive_landmarks();
  if (!lm_graph->use_conjunctive_landmarks()) discard_conjunctive_landmarks();
  ...
  for (unsigned j = 0; j < prepost.size(); j++) {
    for (unsigned int k = 0; k < lmp->vars.size(); k++) {
      if (prepost[j].var == lmp->vars[k] && prepost[j].post == lmp->vals[k])
        if (prepost[j].cond.empty()) return true;
    }
  }
  return false;
}
...
bool LandmarkFactory::is_landmark_precondition(const Operator &o,
  ...
  for (unsigned j = 0; j < prevail.size(); j++) {
    for (unsigned int k = 0; k < lmp->vars.size(); k++) {
      if (prevail[j].var == lmp->vars[k] && prevail[j].prev == lmp->vals[k])
  ...
  for (unsigned j = 0; j < prepost.size(); j++) {
    for (unsigned int k = 0; k < lmp->vars.size(); k++) {
      if (prepost[j].var == lmp->vars[k] && prepost[j].pre == lmp->vals[k])
        // if (prepost[j].cond.empty())
        return true;
    }
  }
...
bool LandmarkFactory::relaxed_task_solvable(
  ...
  /* Test whether the relaxed planning task is solvable without achieving the
   ...
   proposition from "exclude"). As a side effect, collect in lvl_var and lvl_op
   the earliest possible point in time when a proposition / operator can be
   achieved / become applicable in the relaxed task.
   */
  ...
  if (compute_lvl_op) {
    ...
    for (int i = 0; i < g_operators.size() + g_axioms.size(); i++) {
      ...
      for (unsigned j = 0; j < prepost.size(); j++)
        lvl_op[i].insert(make_pair(make_pair(prepost[j].var, prepost[j].post),
                                   numeric_limits<int>::max()));
    }
  }
  lvl_var.resize(g_variable_name.size());
  for (unsigned var = 0; var < g_variable_name.size(); var++) {
    lvl_var[var].resize(g_variable_domain[var], numeric_limits<int>::max());
  }
  ...
  lm_graph->get_exploration()->compute_reachability_with_excludes(
...
bool LandmarkFactory::is_causal_landmark(const LandmarkNode &landmark) const {
  /* Test whether the relaxed planning task is unsolvable without using any
     operator that has "landmark" has a precondition. Similar to
     "relaxed_task_solvable" above.
   */

  if (landmark.in_goal) return true;
  vector<vector<int> > lvl_var;
  vector<hash_map<pair<int, int>, int, hash_int_pair> > lvl_op;
...
bool LandmarkFactory::effect_always_happens(const vector<PrePost> &prepost,
  ...
  /* Test whether the condition of a conditional effect is trivial, i.e. always
   ...
   triggered by a set of conditions of which one will always be true. This is
   e.g. the case in Schedule, where the effect (forall (?oldpaint - colour)
   (when (painted ?x ?oldpaint)
   (not (painted ?x ?oldpaint))))
   is translated by the translator to: if oldpaint == blue, then not painted ?x,
   and if oldpaint == red, then not painted ?x etc. If conditional effects are
   found that are always true, they are returned in "eff".
   */
  // Go through all effects of operator (as given by prepost) and collect:
  // - all variables that are set to some value in a conditional effect
  // (effect_vars)
  // - variables that can be set to more than one value in a cond. effect
  // (nogood_effect_vars)
  // - a mapping from cond. effect propositions to all the conditions that they
  // appear with
  set<int> effect_vars;
    ...
    if (effect_conditions.find(prepost[i].var) != effect_conditions.end() &&
      ...
      for (int k = 0; k < prepost[i].cond.size(); k++) {
        vector<pair<int, int> > &vec =
            effect_conditions.find(prepost[i].var)->second.second;
        vec.push_back(
            make_pair(prepost[i].cond[k].var, prepost[i].cond[k].prev));
      }
    } else {
      ...
      vector<pair<int, int> > &vec =
          effect_conditions
              .insert(make_pair(
                  prepost[i].var,
                  make_pair(prepost[i].post, vector<pair<int, int> >())))
              .first->second.second;
      for (int k = 0; k < prepost[i].cond.size(); k++) {
        vec.push_back(
            make_pair(prepost[i].cond[k].var, prepost[i].cond[k].prev));
  ...
  // For all those effect propositions whose variables do not take on different
  // values...
  ...
  for (; it != effect_conditions.end(); ++it) {
    ...
    for (unsigned int j = 0; j < conds.size(); j++) {
      if (unique_conds.find(conds[j].first) != unique_conds.end()) {
        ...
        set<int> &the_set =
            unique_conds.insert(make_pair(conds[j].first, set<int>()))
                .first->second;
        the_set.insert(conds[j].second);
      }
    }
    // Check for each condition variable whether the number of values it takes
    // on is equal to the domain of that variable...
    ...
    for (; it2 != unique_conds.end(); ++it2) {
      ...
      // ...or else if the condition variable is the same as the effect
      // variable, check whether the condition variable takes on all other
      // values except the effect value
      else if (it2->first == it->first &&
               it2->second.size() == g_variable_domain[it2->first] - 1) {
        // Number of different values is correct, now ensure that the effect
        // value was the one missing
        it2->second.insert(it->second.first);
        if (it2->second.size() == g_variable_domain[it2->first]) {
          is_surely_reached_by_var = true;
        }
      }
      // If one of the condition variables does not fulfill the criteria, the
      // effect is not certain to happen
      if (!is_surely_reached_by_var) is_always_reached = false;
    }
    if (is_always_reached) eff.insert(effect);
  }
  return eff.empty();
}
...
bool LandmarkFactory::interferes(const LandmarkNode *node_a,
  ...
  /* Facts a and b interfere (i.e., achieving b before a would mean having to
   delete b and re-achieve it in order to achieve a) if one of the following
   condition holds:
   1. a and b are mutex
   ...
   formalism), this condition is the same as 2.
   */
  ...
  for (int bi = 0; bi < node_b->vars.size(); bi++) {
    ...
    for (int ai = 0; ai < node_a->vars.size(); ai++) {
      ...
      for (unsigned i = 0; i < ops.size(); i++) {
        ...
        if (!init && shared_eff.empty()) break;
        ...
        // e.g. in Schedule. There, the same effect is conditioned on a
        // disjunction of conditions of which one will always be true. We test
        // for a simple kind of these trivial conditions here.)
        const vector<PrePost> &prepost = op.get_pre_post();
        ...
        if (testing_for_trivial_conditions)
          trivial_conditioned_effects_found =
              effect_always_happens(prepost, trivially_conditioned_effects);
        ...
        for (unsigned i = 0; i < prepost.size(); i++) {
          if (prepost[i].cond.empty() && prepost[i].var != a.first) {
            next_eff.insert(make_pair(prepost[i].var, prepost[i].post));
                     ...
                     trivial_conditioned_effects_found &&
                     trivially_conditioned_effects.find(
                         make_pair(prepost[i].var, prepost[i].post)) !=
        ...
        if (init)
          ...
          for (hash_map<int, int>::iterator it1 = shared_eff.begin();
               it1 != shared_eff.end(); it1++) {
      ...
      for (hash_map<int, int>::iterator it = shared_eff.begin();
        ...
        if (make_pair(it->first, it->second) != a &&
            make_pair(it->first, it->second) != b && are_mutex(*it, b))
          return true;
    }
...
void LandmarkFactory::approximate_reasonable_orders(bool obedient_orders) {
  /* Approximate reasonable and obedient reasonable orders according to Hoffmann
  et al. If flag "obedient_orders" is true, we calculate obedient reasonable
  orders, otherwise reasonable orders.

  If node_p is in goal, then any node2_p which interferes with node_p can be
  reasonably ordered before node_p. Otherwise, if node_p is greedy necessary
  predecessor of node2, and there is another predecessor "parent" of node2, then
  parent and all predecessors of parent can be ordered reasonably before node_p
  if they interfere with node_p.
  */
  for (set<LandmarkNode *>::iterator it = lm_graph->get_nodes().begin();
       it != lm_graph->get_nodes().end(); it++) {
    LandmarkNode *node_p = *it;
    if (node_p->disjunctive) continue;

    if (node_p->is_true_in_state(*g_initial_state)) return;

    if (!obedient_orders && node_p->is_goal()) {
      for (set<LandmarkNode *>::iterator it2 = lm_graph->get_nodes().begin();
           it2 != lm_graph->get_nodes().end(); it2++) {
        LandmarkNode *node2_p = *it2;
        if (node2_p == node_p || node2_p->disjunctive) continue;
      ...
      for (hash_map<LandmarkNode *, edge_type, hash_pointer>::iterator it =
               node_p->children.begin();
           it != node_p->children.end(); it++) {
        if (it->second >=
            greedy_necessary) {  // found node2: node_p ->_gn node2
          LandmarkNode &node2 = *(it->first);
          for (hash_map<LandmarkNode *, edge_type, hash_pointer>::iterator it2 =
                   node2.parents.begin();
               it2 != node2.parents.end(); it2++) {  // find parent
            edge_type &edge = it2->second;
            LandmarkNode &parent = *(it2->first);
            if (parent.disjunctive) continue;
            if ((edge >= natural || (obedient_orders && edge == reasonable)) &&
                &parent !=
                    node_p) {  // find predecessors or parent and collect in
              // "interesting nodes"
      ...
      // Insert reasonable orders between those members of "interesting nodes"
      // that interfere with node_p.
      for (hash_set<LandmarkNode *, hash_pointer>::iterator it3 =
               interesting_nodes.begin();
           it3 != interesting_nodes.end(); it3++) {
        if (*it3 == node_p || (*it3)->disjunctive) continue;
...
void LandmarkFactory::edge_add(LandmarkNode &from, LandmarkNode &to,
                               edge_type type) {
  /* Adds an edge in the landmarks graph if there is no contradicting edge
  (simple measure to reduce cycles. If the edge is already present, the stronger
  edge type wins.
  */
  assert(&from != &to);
  ...
  if (type == reasonable || type == obedient_reasonable) {  // simple cycle test
    if (from.parents.find(&to) !=
        from.parents.end()) {  // Edge in opposite direction exists
      // cout << "edge in opposite direction exists" << endl;
      if (from.parents.find(&to)->second >
          type)  // Stronger order present, return
        return;
...
void LandmarkFactory::discard_noncausal_landmarks() {
  ...
  while (change) {
    change = false;
    for (set<LandmarkNode *>::const_iterator it = lm_graph->get_nodes().begin();
         it != lm_graph->get_nodes().end(); ++it) {
      LandmarkNode *n = *it;
...
void LandmarkFactory::discard_disjunctive_landmarks() {
  ...
  */
  if (lm_graph->number_of_disj_landmarks() == 0) return;
...
void LandmarkFactory::discard_conjunctive_landmarks() {
  if (lm_graph->number_of_conj_landmarks() == 0) return;
...
bool LandmarkFactory::remove_first_weakest_cycle_edge(
    LandmarkNode *cur, list<pair<LandmarkNode *, edge_type> > &path,
    list<pair<LandmarkNode *, edge_type> >::iterator it) {
  LandmarkNode *parent_p = 0;
  LandmarkNode *child_p = 0;
  for (list<pair<LandmarkNode *, edge_type> >::iterator it2 = it;
       it2 != path.end(); it2++) {
    edge_type edge = it2->second;
    if (edge == reasonable || edge == obedient_reasonable) {
      ...
      if (edge == obedient_reasonable) break;
...
int LandmarkFactory::loop_acyclic_graph(
  ...
  hash_set<LandmarkNode *, hash_pointer> visited =
      hash_set<LandmarkNode *, hash_pointer>(lm_graph->number_of_landmarks());
...
void LandmarkFactory::compute_predecessor_information(
    LandmarkNode *bp, vector<vector<int> > &lvl_var,
    vector<hash_map<pair<int, int>, int, hash_int_pair> > &lvl_op) {
...
void LandmarkFactory::calc_achievers() {
  for (set<LandmarkNode *>::iterator node_it = lm_graph->get_nodes().begin();
       node_it != lm_graph->get_nodes().end(); ++node_it) {
    LandmarkNode &lmn = **node_it;

    for (int k = 0; k < lmn.vars.size(); k++) {
      vector<int> ops = lm_graph->get_operators_including_eff(
          make_pair(lmn.vars[k], lmn.vals[k]));
      lmn.possible_achievers.insert(ops.begin(), ops.end());

      if (g_axiom_layers[lmn.vars[k]] != -1) lmn.is_derived = true;
    }
    ...
    for (ach_it = lmn.possible_achievers.begin();
         ach_it != lmn.possible_achievers.end(); ++ach_it) {
      int op_id = *ach_it;
