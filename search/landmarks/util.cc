...
bool _possibly_fires(const vector<Prevail> &prevail,
  ...
  for (int j = 0; j < prevail.size(); j++)
    if (lvl_var[prevail[j].var][prevail[j].prev] == numeric_limits<int>::max())
      return false;
  return true;
}
...
hash_map<int, int> _intersect(const hash_map<int, int> &a,
  ...
  if (a.size() > b.size()) return _intersect(b, a);
  hash_map<int, int> result;
  for (hash_map<int, int>::const_iterator it1 = a.begin(); it1 != a.end();
       it1++) {
    hash_map<int, int>::const_iterator it2 = b.find(it1->first);
    if (it2 != b.end() && it2->second == it1->second) result.insert(*it1);
  }
  return result;
}
...
bool _possibly_reaches_lm(const Operator &o,
  ...
  for (unsigned i = 0; i < prevail.size(); i++)
    if (lvl_var[prevail[i].var][prevail[i].prev] == numeric_limits<int>::max())
      return false;
  ...
  for (unsigned i = 0; i < prepost.size(); i++) {
    ...
    // Otherwise, operator is not applicable
    if (prepost[i].pre != -1)
      if (lvl_var[prepost[i].var][prepost[i].pre] == numeric_limits<int>::max())
        return false;
    ...
    if (correct_effect)
      ...
      continue;
    if (effect_condition_reachable) reaches_lm = true;
  }
