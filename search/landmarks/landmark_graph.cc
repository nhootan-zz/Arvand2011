...
LandmarkNode *LandmarkGraph::get_landmark(const pair<int, int> &prop) const {
  /* Return pointer to landmark node that corresponds to the given fact, or 0 if
   no such landmark exists.
   */
  ...
  if (it != simple_lms_to_nodes.end())
    ...
    if (it2 != disj_lms_to_nodes.end()) node_p = it2->second;
  }
  return node_p;
}
...
bool LandmarkGraph::landmark_exists(const pair<int, int> &lm) const {
  // Note: this only checks for one fact whether it's part of a landmark, hence
  // only simple and disjunctive landmarks are checked.
...
bool LandmarkGraph::disj_landmark_exists(const set<pair<int, int> > &lm) const {
  ...
  for (set<pair<int, int> >::const_iterator it = lm.begin(); it != lm.end();
       ++it) {
    const pair<int, int> &prop = *it;
    hash_map<pair<int, int>, LandmarkNode *, hash_int_pair>::const_iterator
        it2 = disj_lms_to_nodes.find(prop);
    if (it2 != disj_lms_to_nodes.end()) return true;
  }
  return false;
}
...
LandmarkNode &LandmarkGraph::landmark_add_conjunctive(
    const set<pair<int, int> > &lm) {
  vector<int> vars;
  vector<int> vals;
  for (set<pair<int, int> >::iterator it = lm.begin(); it != lm.end(); ++it) {
...
void LandmarkGraph::set_landmark_ids() {
  ...
  int id = 0;
  for (set<LandmarkNode *>::iterator node_it = nodes.begin();
...
void LandmarkGraph::dump_node(const LandmarkNode *node_p) const {
  ...
  for (unsigned int i = 0; i < node_p->vars.size(); i++) {
    ...
    cout << g_fact_names[var_no][value] << " (" << g_variable_name[var_no]
         << "(" << var_no << ")"
         << "->" << value << ")";
    if (i < node_p->vars.size() - 1) cout << ", ";
  }
  if (node_p->disjunctive || node_p->conjunctive) cout << "}";
  if (node_p->in_goal) cout << "(goal)";
  cout << " Achievers (" << node_p->possible_achievers.size() << ", "
       << node_p->first_achievers.size() << ")";
  cout << endl;
}
...
void LandmarkGraph::dump() const {
  ...
  for (set<LandmarkNode *>::const_iterator it = nodes2.begin();
       it != nodes2.end(); it++) {
    LandmarkNode *node_p = *it;
    dump_node(node_p);
    for (hash_map<LandmarkNode *, edge_type, hash_pointer>::const_iterator
             parent_it = node_p->parents.begin();
         parent_it != node_p->parents.end(); parent_it++) {
    ...
    for (hash_map<LandmarkNode *, edge_type, hash_pointer>::const_iterator
             child_it = node_p->children.begin();
         child_it != node_p->children.end(); child_it++) {
...
void LandmarkGraph::add_options_to_parser(OptionParser &parser) {
  ...
  parser.add_option<bool>("reasonable_orders", false,
                          "generate reasonable orders");
  parser.add_option<bool>("only_causal_landmarks", false,
                          "keep only causal landmarks");
  parser.add_option<bool>("disjunctive_landmarks", true,
                          "keep disjunctive landmarks");
  parser.add_option<bool>("conjunctive_landmarks", true,
                          "keep conjunctive landmarks");
  parser.add_option<bool>("no_orders", false, "discard all orderings");
  vector<string> cost_types;
  cost_types.push_back("NORMAL");
  cost_types.push_back("ONE");
  cost_types.push_back("PLUSONE");
  parser.add_enum_option("lm_cost_type", cost_types, "NORMAL",
