...
Exploration::Exploration(const Options &opts)
    : Heuristic(opts), did_write_overflow_warning(false) {
  ...
  for (int i = 0; i < g_axioms.size(); i++) build_unary_operators(g_axioms[i]);
...
void Exploration::set_additional_goals(
    const std::vector<pair<int, int> > &add_goals) {
  // Clear previous additional goals.
  for (int i = 0; i < termination_propositions.size(); i++) {
    int var = termination_propositions[i]->var,
        val = termination_propositions[i]->val;
...
void Exploration::build_unary_operators(const Operator &op) {
  ...
  for (int i = 0; i < prevail.size(); i++) {
    ...
    assert(prevail[i].prev >= 0 &&
           prevail[i].prev < g_variable_domain[prevail[i].var]);
    // precondition.push_back(&propositions[prevail[i].var][prevail[i].prev]);
    precondition_var_vals1.push_back(
        make_pair(prevail[i].var, prevail[i].prev));
  }
  for (int i = 0; i < pre_post.size(); i++) {
    if (pre_post[i].pre != -1) {
      assert(pre_post[i].var >= 0 &&
             pre_post[i].var < g_variable_domain.size());
      assert(pre_post[i].pre >= 0 &&
             pre_post[i].pre < g_variable_domain[pre_post[i].var]);
      // precondition.push_back(&propositions[pre_post[i].var][pre_post[i].pre]);
      precondition_var_vals1.push_back(
          make_pair(pre_post[i].var, pre_post[i].pre));
    }
  }
  for (int i = 0; i < pre_post.size(); i++) {
    ...
    assert(pre_post[i].post >= 0 &&
           pre_post[i].post < g_variable_domain[pre_post[i].var]);
    ...
    for (int j = 0; j < eff_cond.size(); j++) {
      assert(eff_cond[j].var >= 0 &&
             eff_cond[j].var < g_variable_domain.size());
      assert(eff_cond[j].prev >= 0 &&
             eff_cond[j].prev < g_variable_domain[eff_cond[j].var]);
      // precondition.push_back(&propositions[eff_cond[j].var][eff_cond[j].prev]);
      precondition_var_vals2.push_back(
          make_pair(eff_cond[j].var, eff_cond[j].prev));
    }
    ...
    unary_operators.push_back(
        ExUnaryOperator(precondition, effect, &op, base_cost));
    // precondition.erase(precondition.end() - eff_cond.size(),
    // precondition.end());
    precondition.clear();
...
class hash_unary_operator {
 public:
  size_t operator()(
      const pair<vector<ExProposition *>, ExProposition *> &key) const {
...
void Exploration::setup_exploration_queue(
    const State &state, const vector<pair<int, int> > &excluded_props,
  ...
  if (excluded_props.size() > 0) {
    for (unsigned i = 0; i < excluded_props.size(); i++) {
      ExProposition &prop =
          propositions[excluded_props[i].first][excluded_props[i].second];
      prop.h_add_cost = -2;
    }
  }
...
void Exploration::relaxed_exploration(bool use_h_max = false,
  ...
  while (!prop_queue.empty()) {
    ...
    for (int i = 0; i < triggered_operators.size(); i++) {
      ...
      if (unary_op->unsatisfied_preconditions == 0) {
        int depth =
            unary_op->op->is_axiom() ? unary_op->depth : unary_op->depth + 1;
        if (use_h_max)
...
int Exploration::compute_ff_heuristic(const State &state) {
  ...
  if (h_add_heuristic == DEAD_END) {
    ...
    for (; it != relaxed_plan.end(); ++it) cost += get_adjusted_cost(**it);
    return cost;
  }
}
  ...
  if (!goal->marked) {  // Only consider each subgoal once.
    ...
    if (unary_op) {  // We have not yet chained back to a start node.
      ...
      if (added_to_relaxed_plan &&
          unary_op->h_add_cost == unary_op->base_cost && unary_op->depth == 0 &&
          !op->is_axiom()) {
        set_preferred(op);
...
void Exploration::compute_reachability_with_excludes(
    ...
    bool level_out, const vector<pair<int, int> > &excluded_props,
    const hash_set<const Operator *, ex_hash_operator_ptr> &excluded_ops,
  ...
  for (int var = 0; var < propositions.size(); var++) {
    for (int value = 0; value < propositions[var].size(); value++) {
      ExProposition &prop = propositions[var][value];
      if (prop.h_max_cost >= 0) lvl_var[var][value] = prop.h_max_cost;
    }
  }
  if (compute_lvl_ops) {
...
void Exploration::collect_ha(ExProposition *goal, RelaxedPlan &relaxed_plan,
                             const State &state) {
  ...
  if (unary_op) {  // We have not yet chained back to a start node.
    ...
    const Operator *op = unary_op->op;
    bool added_to_relaxed_plan = false;
    if (!op->is_axiom()) added_to_relaxed_plan = relaxed_plan.insert(op).second;
    if (added_to_relaxed_plan && unary_op->h_add_cost == unary_op->base_cost &&
        unary_op->depth == 0 && !op->is_axiom()) {
...
bool is_landmark(vector<pair<int, int> > &landmarks, int var, int val) {
  ...
  for (int i = 0; i < landmarks.size(); i++)
    if (landmarks[i].first == var && landmarks[i].second == val) return true;
  return false;
}
...
bool Exploration::plan_for_disj(vector<pair<int, int> > &landmarks,
                                const State &state) {
  relaxed_plan.clear();
  // generate plan to reach part of disj. goal OR if no landmarks given, plan to
  // real goal
  if (!landmarks.empty()) {
    ...
    for (int i = 0; i < termination_propositions.size(); i++) {
      ...
      if (prop_cost == -1 &&
          is_landmark(landmarks, termination_propositions[i]->var,
    ...
    for (int i = 0; i < goal_propositions.size(); i++) {
      if (goal_propositions[i]->h_add_cost == -1) return false;  // dead end
