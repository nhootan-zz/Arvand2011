...
void LandmarkStatusManager::clear_reached() { reached_lms.clear(); }
...
void LandmarkStatusManager::set_landmarks_for_initial_state() {
  ...
  for (set<LandmarkNode *>::iterator it = nodes.begin(); it != nodes.end();
       it++) {
  ...
  cout << inserted << " initial landmarks, " << num_goal_lms
       << " goal landmarks" << endl;
}

bool LandmarkStatusManager::update_reached_lms(const State &parent_state,
                                               const Operator &,
                                               const State &state) {
...
bool LandmarkStatusManager::update_lm_status(const State &state) {
  ...
  for (lit = nodes.begin(); lit != nodes.end(); lit++) {
    ...
    //
    // Note: this only tests for reachability of the landmark from the initial
    // state. A (possibly) more effective option would be to test reachability
    // of the landmark from the current state.
...
bool LandmarkStatusManager::landmark_is_leaf(
    const LandmarkNode &node, const vector<bool> &reached) const {
