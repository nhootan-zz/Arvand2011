...
void HMLandmarks::get_m_sets_of_set_(int m, int num_included,
  ...
  for (int i = 0; i < current.size(); i++) {
    if (!interesting(superset[current_var_index].first,
                     superset[current_var_index].second, current[i].first,
                     current[i].second)) {
      use_var = false;
      break;
    }
  ...
  if (use_var) {
    ...
    get_m_sets_of_set_(m, num_included + 1, current_var_index + 1, current,
                       subsets, superset);
    current.pop_back();
  }

  // don't include current fluent in set
  get_m_sets_of_set_(m, num_included, current_var_index + 1, current, subsets,
                     superset);
}

// get subsets of superset1 \cup superset2 with size m or less,
// such that they have >= 1 elements from each set.
void HMLandmarks::get_split_m_sets_(int m, int ss1_num_included,
                                    int ss2_num_included, int ss1_var_index,
                                    int ss2_var_index, FluentSet &current,
                                    std::vector<FluentSet> &subsets,
  ...
  if ((ss1_var_index != superset1.size()) &&
    ...
    for (int i = 0; i < current.size(); i++) {
      if (!interesting(superset1[ss1_var_index].first,
                       superset1[ss1_var_index].second, current[i].first,
                       current[i].second)) {
        use_var = false;
        break;
      }
    ...
    if (use_var) {
      ...
      get_split_m_sets_(m, ss1_num_included + 1, ss2_num_included,
                        ss1_var_index + 1, ss2_var_index, current, subsets,
                        superset1, superset2);
      current.pop_back();
    }

    // don't include
    get_split_m_sets_(m, ss1_num_included, ss2_num_included, ss1_var_index + 1,
                      ss2_var_index, current, subsets, superset1, superset2);
  } else {
    for (int i = 0; i < current.size(); i++) {
      if (!interesting(superset2[ss2_var_index].first,
                       superset2[ss2_var_index].second, current[i].first,
                       current[i].second)) {
        use_var = false;
        break;
      }
    ...
    if (use_var) {
      ...
      get_split_m_sets_(m, ss1_num_included, ss2_num_included + 1,
                        ss1_var_index, ss2_var_index + 1, current, subsets,
                        superset1, superset2);
      current.pop_back();
    }

    // don't include
    get_split_m_sets_(m, ss1_num_included, ss2_num_included, ss1_var_index,
                      ss2_var_index + 1, current, subsets, superset1,
                      superset2);
  }
}

// use together is method that determines whether the two variables are
// interesting together, e.g. we don't want to represent (truck1-loc x,
// truck2-loc y) type stuff
...
// assume disjoint
void HMLandmarks::get_split_m_sets(int m, std::vector<FluentSet> &subsets,
                                   const FluentSet &superset1,
                                   const FluentSet &superset2) {
  FluentSet c;
...
bool HMLandmarks::possible_noop_set(const FluentSet &fs1,
                                    const FluentSet &fs2) {
  ...
  for (fs1it = fs1.begin(); fs1it != fs1.end(); ++fs1it) {
    for (fs2it = fs2.begin(); fs2it != fs2.end(); ++fs2it) {
      if (are_mutex(make_pair(fs1it->first, fs1it->second),
                    make_pair(fs2it->first, fs2it->second)))
        return false;
    }
  }
...
void HMLandmarks::build_pm_ops() {
  FluentSet pc, eff;
  std::vector<FluentSet> pc_subsets, eff_subsets, noop_pc_subsets,
      noop_eff_subsets;
  ...
  for (int i = 0; i < g_operators.size(); i++) {
    ...
    while ((it->first.size() < m_) && (it != set_indices_.end())) {
      if (possible_noop_set(eff, it->first)) {
        ...
        this_cond_noop.reserve(noop_pc_subsets.size() +
                               noop_eff_subsets.size() + 1);
...
HMLandmarks::HMLandmarks(const Options &opts)
    : LandmarkFactory(opts), m_(opts.get<int>("m")) {
...
void HMLandmarks::init() {
  ...
  std::cout << "Using " << h_m_table_.size() << " P^m fluents." << std::endl;

  // unsatisfied pc counts are now in build pm ops

  build_pm_ops();
  //  std::cout << "Built P(m) ops, total: " << pm_ops_.size() << "." <<
  //  std::endl;
}
...
void HMLandmarks::calc_achievers() {
  ...
  for (std::set<LandmarkNode *>::iterator it = lm_graph->get_nodes().begin();
    ...
    for (int i = 0; i < lmn.vars.size(); i++) {
      const std::vector<int> &ops = lm_graph->get_operators_including_eff(
          std::make_pair(lmn.vars[i], lmn.vals[i]));
    ...
    for (std::set<int>::iterator cands_it = candidates.begin();
      ...
      for (j = 0; j < lmn.vars.size(); j++) {
        ...
        if (std::find(eff.begin(), eff.end(), lm_val) != eff.end()) continue;
        int k;
  ...
  for (int i = 0; i < h_m_table_[factindex].pc_for.size(); i++) {
    ...
    if (info.second == -1) {
      ...
      if (unsat_pc_count_[info.first].first == 0) {
        // create empty set or clear prev entries -- signals do all possible
        // noop effects
    ...
    // a pc for a conditional noop
      ...
      // (if associated action is not applicable, all noops will be used when it
      // first does)
      if ((unsat_pc_count_[info.first].first == 0) &&
...
void HMLandmarks::compute_h_m_landmarks() {
  ...
  while (!current_trigger.empty()) {
    for (op_it = current_trigger.begin(); op_it != current_trigger.end();
         ++op_it) {
      ...
      if (op_it->second.empty()) {
        for (int i = 0; i < action.cond_noops.size(); i++) {
          ...
          if (unsat_pc_count_[op_index].second[i] == 0) {
            compute_noop_landmarks(op_index, i, local_landmarks,
                                   local_necessary, level, next_trigger);
          }
        }
      }
      ...
      // landmarks have changed
        ...
        for (std::set<int>::iterator noop_it = op_it->second.begin();
          ...
          compute_noop_landmarks(op_index, *noop_it, local_landmarks,
                                 local_necessary, level, next_trigger);
        }
      }
    }
...
void HMLandmarks::compute_noop_landmarks(int op_index, int noop_index,
                                         std::list<int> const &local_landmarks,
                                         std::list<int> const &local_necessary,
                                         int level, TriggerSet &next_trigger) {
...
void HMLandmarks::generate_landmarks() {
  ...
  for (int i = 0; i < goal_subsets.size(); i++) {
    ...
    if (h_m_table_[set_index].level == -1) {
      std::cout << std::endl
                << std::endl
                << "Subset of goal not reachable !!." << std::endl
                << std::endl
                << std::endl;
      std::cout << "Subset is: ";
  ...
  for (std::list<int>::iterator it = all_lms.begin(); it != all_lms.end();
       ++it) {
  ...
  if (lm_graph->use_orders()) {
    ...
    for (std::list<int>::iterator f1 = all_lms.begin(); f1 != all_lms.end();
         ++f1) {
    ...
    for (std::list<int>::iterator it = all_lms.begin(); it != all_lms.end();
         ++it) {
      set_index = *it;
      for (std::list<int>::iterator lms_it =
               h_m_table_[set_index].landmarks.begin();
      ...
      if (lm_graph->use_orders()) {
        for (std::list<int>::iterator gn_it =
                 h_m_table_[set_index].necessary.begin();
             gn_it != h_m_table_[set_index].necessary.end(); ++gn_it) {
          edge_add(*lm_node_table_[*gn_it], *lm_node_table_[set_index],
                   greedy_necessary);
        }
      }
    }
...
static Plugin<LandmarkGraph> _plugin("lm_hm", _parse);
