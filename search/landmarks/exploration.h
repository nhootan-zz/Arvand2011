...
class Exploration : public Heuristic {
  ...
  typedef __gnu_cxx::hash_set<const Operator *, ex_hash_operator_ptr>
      RelaxedPlan;
  ...
  void setup_exploration_queue(
      ...
      const __gnu_cxx::hash_set<const Operator *, ex_hash_operator_ptr>
          &excluded_ops,
      bool use_h_max);
  ...
  void collect_relaxed_plan(ExProposition *goal, RelaxedPlan &relaxed_plan,
                            const State &state);
  ...
  void collect_ha(ExProposition *goal, RelaxedPlan &relaxed_plan,
                  const State &state);
 ...
 public:
  ...
  void compute_reachability_with_excludes(
      ...
      std::vector<__gnu_cxx::hash_map<std::pair<int, int>, int, hash_int_pair> >
          &lvl_op,
      bool level_out, const std::vector<std::pair<int, int> > &excluded_props,
      const __gnu_cxx::hash_set<const Operator *, ex_hash_operator_ptr>
          &excluded_ops,
      bool compute_lvl_ops);
  std::vector<const Operator *>
      exported_ops;  // only needed for landmarks count heuristic ha

  // Returns true iff disj_goal is relaxed reachable. As a side effect, marks
  // preferred operators via "exported_ops". (This is the real reason why you
  // might want to call this.)
  bool plan_for_disj(std::vector<std::pair<int, int> > &disj_goal,
                     const State &state);
