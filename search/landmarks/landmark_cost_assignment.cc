...
double LandmarkUniformSharedCostAssignment::cost_sharing_h_value() {
  ...
  for (node_it = nodes.begin(); node_it != nodes.end(); ++node_it) {
    ...
    if (lmn_status != lm_reached) {
      ...
      if (use_action_landmarks && achievers.size() == 1) {
        ...
        for (ach_it = achievers.begin(); ach_it != achievers.end(); ++ach_it) {
          int op_id = *ach_it;
          assert(op_id >= 0 && op_id < g_operators.size());
  ...
  for (int i = 0; i < relevant_lms.size(); i++) {
    ...
    for (ach_it = achievers.begin(); ach_it != achievers.end(); ++ach_it) {
      ...
      double shared_cost =
          double(get_adjusted_action_cost(op, cost_type)) / num_achieved;
...
LandmarkEfficientOptimalSharedCostAssignment::
    LandmarkEfficientOptimalSharedCostAssignment(LandmarkGraph &graph,
...
double LandmarkEfficientOptimalSharedCostAssignment::cost_sharing_h_value() {
  ...
  try {
    ...
    CoinPackedMatrix *matrix = new CoinPackedMatrix(
        false, &*operator_indices.begin(), &*landmark_indices.begin(),
        &*coefficients.begin(), num_constraints);
