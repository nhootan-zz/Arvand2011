...
struct HMEntry {
  ...
  std::list<int>
      necessary;  // greedy necessary landmarks, disjoint from landmarks
...
class HMLandmarks : public LandmarkFactory {
 ...
 private:
  ...
  void compute_noop_landmarks(int op_index, int noop_index,
                              ...
                              std::list<int> const &local_necessary, int level,
                              TriggerSet &next_trigger);

  void propagate_pm_fact(int fact, bool newly_discovered, TriggerSet &trigger);
  ...
  void get_m_sets_(int m, int num_included, int current_var, FluentSet &current,
  ...
  void get_split_m_sets_(int m, int ss1_num_included, int ss2_num_included,
  ...
  void get_m_sets(int m, std::vector<FluentSet> &subsets, const State &s);
