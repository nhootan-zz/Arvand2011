...
bool LandmarkFactoryZhuGivan::satisfies_goal_conditions(
  ...
  for (unsigned i = 0; i < g_goal.size(); i++)
    if (!layer[g_goal[i].first][g_goal[i].second].reached()) return false;
...
void LandmarkFactoryZhuGivan::extract_landmarks(
  ...
  for (unsigned i = 0; i < g_goal.size(); i++) {
    ...
    for (lm_set::const_iterator it = goal_node.labels.begin();
         it != goal_node.labels.end(); it++) {
...
LandmarkFactoryZhuGivan::proposition_layer
LandmarkFactoryZhuGivan::build_relaxed_plan_graph_with_labels() const {
  ...
  while (changes) {
    ...
    changes = false;
    for (hash_set<int>::const_iterator it = triggered.begin();
         it != triggered.end(); it++) {
      ...
      if (operator_applicable(op, current_prop_layer)) {
        ...
        if (!changed.empty()) {
          changes = true;
          for (lm_set::const_iterator it2 = changed.begin();
               it2 != changed.end(); it2++)
            next_triggered.insert(triggers[it2->first][it2->second].begin(),
...
bool LandmarkFactoryZhuGivan::operator_applicable(
    const Operator &op, const proposition_layer &state) const {
  // test preconditions
  const vector<Prevail> &prevail = op.get_prevail();
  for (unsigned i = 0; i < prevail.size(); i++)
    if (!state[prevail[i].var][prevail[i].prev].reached()) return false;
...
bool LandmarkFactoryZhuGivan::operator_cond_effect_fires(
  ...
  for (unsigned i = 0; i < cond.size(); i++)
    if (!state[cond[i].var][cond[i].prev].reached()) return false;
  return true;
}
...
lm_set LandmarkFactoryZhuGivan::union_of_precondition_labels(
    const Operator &op, const proposition_layer &current) const {
  lm_set result;

  const vector<Prevail> &prevail = op.get_prevail();
  for (unsigned i = 0; i < prevail.size(); i++)
    result = _union(result, current[prevail[i].var][prevail[i].prev].labels);

  const vector<PrePost> &prepost = op.get_pre_post();
  for (unsigned i = 0; i < prepost.size(); i++)
    if (prepost[i].pre != -1)
      result = _union(result, current[prepost[i].var][prepost[i].pre].labels);
...
static bool _propagate_labels(lm_set &labels, const lm_set &new_labels,
  ...
  // by intersection.
  if (old_labels.size() != labels.size()) return true;
...
lm_set LandmarkFactoryZhuGivan::apply_operator_and_propagate_labels(
  ...
  for (int i = 0; i < prepost.size(); i++) {
    ...
    if (next[var][post].labels.size() == 1) continue;

    if (operator_cond_effect_fires(prepost[i].cond, current)) {
      const lm_set precond_label_union_with_condeff =
          _union(precond_label_union,
                 union_of_condition_labels(
                     prepost[i].cond,
                     current));  // NOTE: this equals precond_label_union, if
                                 // prepost[i] is
