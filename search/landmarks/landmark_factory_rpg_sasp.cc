...
void LandmarkFactoryRpgSasp::get_greedy_preconditions_for_lm(
    const LandmarkNode *lmp, const Operator &o,
    hash_map<int, int> &result) const {
  // Computes a subset of the actual preconditions of o for achieving lmp -
    ...
    if (prepost[j].pre != -1) {
      ...
      for (int i = 0; i < lmp->vars.size(); i++) {
        if (lmp->vars[i] == prepost[j].var &&
          ...
          result.insert(
              make_pair(prepost[j].var, (*g_initial_state)[prepost[j].var]));
          break;
        }
      }
    }
      ...
      if (lmp->vars[*it] == prepost[j].var &&
        ...
        if (prepost[j].cond.empty()) {
          ...
          for (unsigned k = 0; k < prepost[j].cond.size(); k++)
            current_cond.insert(
                make_pair(prepost[j].cond[k].var, prepost[j].cond[k].prev));
      }
      if (init) {
        init = false;
  ...
  for (unsigned int k = 0; k < bp->vars.size(); k++) {
    ...
    for (unsigned i = 0; i < ops.size(); i++) {
      ...
      if (_possibly_reaches_lm(op, lvl_var, bp))
        min_cost = min(min_cost, get_adjusted_action_cost(
                                     op, lm_graph->get_lm_cost_type()));
    }
  }
...
void LandmarkFactoryRpgSasp::found_simple_lm_and_order(const pair<int, int> a,
  ...
  if (lm_graph->disj_landmark_exists(a_set)) {
    ...
    /* TODO: Problem: Schon diese jetzige Implementierung ist nicht mehr
    korrekt, da rm_landmark_node nicht nur bei allen children die parents-zeiger
    auf sich selbst löscht, sondern auch bei allen parents die children-zeiger
    auf sich selbst. Ein einfaches Speichern aller Attribute von node
    funktioniert also nicht - entweder man muss dann manuell bei den parents des
    alten node alle children-Zeiger neu setzen auf den neuen node oder man
    überarbeitet das ganze komplett anders... Eine andere Vermutung meinerseits
    wäre, dass die alte Version verbugt ist und eigentlich auch die children-
    Zeiger der parents von node gelöscht werden müssten, wie es in
    rm_landmark_node passiert.
    */
    // TODO: avoid copy constructor, save attributes locally and assign to new
    // lm new: replace by new program logic
...
void LandmarkFactoryRpgSasp::found_disj_lm_and_order(
    const set<pair<int, int> > a, LandmarkNode &b, edge_type t) {
...
static string get_predicate_for_fact(int var_no, int value) {
  ...
  if (fact_name.substr(0, 5) != "Atom " || paren_pos == string::npos) {
    cerr << "error: cannot extract predicate from fact: " << fact_name << endl;
    exit(1);
  }
...
void LandmarkFactoryRpgSasp::compute_disjunctive_preconditions(
  ...
  for (int i = 0; i < bp->vars.size(); i++) {
    ...
    for (int j = 0; j < tmp_ops.size(); j++) ops.push_back(tmp_ops[j]);
  }
  int no_ops = 0;
  ...
  for (unsigned i = 0; i < ops.size(); i++) {
    ...
    if (_possibly_reaches_lm(op, lvl_var, bp)) {
      ...
      hash_map<int, int> next_pre;
      get_greedy_preconditions_for_lm(bp, op, next_pre);
      for (hash_map<int, int>::iterator it = next_pre.begin();
           it != next_pre.end(); it++) {
  ...
  for (hash_map<int, vector<pair<int, int> > >::iterator it =
    ...
    if (it->second.size() == no_ops) {
      ...
      if (pre_set.size() >
          1)  // otherwise this LM is not actually a disjunctive LM
...
void LandmarkFactoryRpgSasp::generate_landmarks() {
  ...
  while (!open_landmarks.empty()) {
    ...
    if (!bp->is_true_in_state(*g_initial_state)) {
      ...
      // Use this information to determine all operators that can possibly
      // achieve bp for the first time, and collect any precondition
      // propositions that all such operators share (if there are any).
      hash_map<int, int> shared_pre;
      compute_shared_preconditions(shared_pre, lvl_var, bp);
      // All such shared preconditions are landmarks, and greedy necessary
      // predecessors of bp.
      for (hash_map<int, int>::iterator it = shared_pre.begin();
           it != shared_pre.end(); it++) {
      ...
      // Use the information about possibly achieving operators of bp to set its
      // min cost.
      ...
      for (int i = 0; i < disjunctive_pre.size(); i++)
        if (disjunctive_pre[i].size() <
            5) {  // We don't want disj. LMs to get too big
...
void LandmarkFactoryRpgSasp::approximate_lookahead_orders(
  ...
  // Use domain transition graphs to find further orders. Only possible if lmp
  // is a simple landmark.
  if (lmp->disjunctive) return;
  ...
  for (int i = 0; i < g_variable_domain[lmk.first]; i++)
    if (lvl_var[lmk.first][i] == numeric_limits<int>::max() && lmk.second != i)
      unreached.insert(i);
...
bool LandmarkFactoryRpgSasp::domain_connectivity(const pair<int, int> &landmark,
  ...
  /* Tests whether in the domain transition graph of the LM variable, there is
   ...
   any value in "exclude". If not, that means that one of the values in
   "exclude" is crucial for achieving the landmark (i.e. is on every path to the
   LM).
   */
  assert(landmark.second !=
         (*g_initial_state)[landmark.first]);  // no initial state landmarks
...
void LandmarkFactoryRpgSasp::find_forward_orders(
    const vector<vector<int> > &lvl_var, LandmarkNode *lmp) {
  /* lmp is ordered before any var-val pair that cannot be reached before lmp
   according to relaxed planning graph (as captured in lvl_var). These orders
   are saved in the node member variable "forward_orders".
   */
  for (int i = 0; i < g_variable_domain.size(); i++)
    for (int j = 0; j < g_variable_domain[i]; j++) {
      if (lvl_var[i][j] != numeric_limits<int>::max()) continue;
      ...
      for (int m = 0; m < lmp->vars.size() && insert; m++) {
        ...
        if (make_pair(i, j) != lm) {
          // Make sure there is no operator that reaches both lm and (i, j) at
          // the same time
          bool intersection_empty = true;
          const vector<int> &reach_ij =
              lm_graph->get_operators_including_eff(make_pair(i, j));
