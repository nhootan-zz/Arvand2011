...
class LandmarkFactoryRpgSasp : public LandmarkFactory {
  ...
  void get_greedy_preconditions_for_lm(
      ...
      __gnu_cxx::hash_map<int, int> &result) const;
  void compute_shared_preconditions(__gnu_cxx::hash_map<int, int> &shared_pre,
  ...
  int min_cost_for_landmark(LandmarkNode *bp,
                            std::vector<std::vector<int> > &lvl_var);
  void generate_landmarks();
  void found_simple_lm_and_order(const std::pair<int, int> a, LandmarkNode &b,
                                 edge_type t);
  void found_disj_lm_and_order(const std::set<std::pair<int, int> > a,
                               LandmarkNode &b, edge_type t);
  void approximate_lookahead_orders(
      const std::vector<std::vector<int> > &lvl_var, LandmarkNode *lmp);
