...
LandmarkNode *LandmarkGraphMerged::get_matching_landmark(
...
void LandmarkGraphMerged::generate_landmarks() {
  ...
  for (int i = 0; i < lm_graphs.size(); i++) {
    ...
    for (it = nodes.begin(); it != nodes.end(); it++) {
      ...
      if (!node.conjunctive && !node.disjunctive &&
          !lm_graph->landmark_exists(lm_fact)) {
  ...
  for (int i = 0; i < lm_graphs.size(); i++) {
    ...
    for (it = nodes.begin(); it != nodes.end(); it++) {
      ...
      if (from) {
        ...
        for (to_it = from_orig.children.begin();
             to_it != from_orig.children.end(); to_it++) {
...
static Plugin<LandmarkGraph> _plugin("lm_merged", _parse);
