...
extern LandmarkGraph
    *g_lgraph;  // Make global so graph does not need to be built more than once
...
class LandmarkCountHeuristic : public Heuristic {
  ...
  void collect_lm_leaves(bool disjunctive_lms, LandmarkSet &result,
  ...
  bool generate_helpful_actions(const State &state, const LandmarkSet &reached);
