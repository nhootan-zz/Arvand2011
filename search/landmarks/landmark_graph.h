...
class LandmarkNode {
 ...
 public:
  LandmarkNode(std::vector<int> &variables, std::vector<int> &values, bool disj,
               bool conj = false)
      : id(-1),
        vars(variables),
        vals(values),
...
struct LandmarkNodeComparer {
  bool operator()(LandmarkNode *a, LandmarkNode *b) const {
    ...
    for (int i = 0; i < a->vars.size(); i++) {
      if (a->vars[i] > b->vars[i]) return true;
...
class LandmarkGraph {
 public:
  ...
  // ------------------------------------------------------------------------------
  // methods needed by both landmarkgraph-factories and
  // non-landmarkgraph-factories
  inline const set<LandmarkNode *> &get_nodes() const { return nodes; }
  inline const Operator &get_operator_for_lookup_index(int op_no) const {
    const Operator &op = (op_no < g_operators.size())
                             ? g_operators[op_no]
                             : g_axioms[op_no - g_operators.size()];
    return op;
  }
  ...
  inline LandmarkNode &get_disj_lm_node(const pair<int, int> &a) const {
    // Note: this only works because every proposition appears in only one disj.
    // LM
    assert(!simple_landmark_exists(a));
    assert(disj_lms_to_nodes.find(a) != disj_lms_to_nodes.end());
    return *(disj_lms_to_nodes.find(a)->second);
  }
  inline const std::vector<int> &get_operators_including_eff(
      const pair<int, int> &eff) const {
  ...
  int number_of_conj_landmarks() const { return conj_lms; }
  int number_of_edges() const;

  // HACK! (Temporary accessor needed for LandmarkFactorySasp.)
  OperatorCost get_lm_cost_type() const { return lm_cost_type; }

  bool simple_landmark_exists(
      const pair<int, int> &lm) const;  // not needed by HMLandmark
  bool disj_landmark_exists(
      const set<pair<int, int> > &lm) const;  // not needed by HMLandmark
  bool landmark_exists(
      const pair<int, int> &lm) const;  // not needed by HMLandmark
  ...
  LandmarkNode &make_disj_node_simple(
      std::pair<int, int> lm);  // only needed by LandmarkFactorySasp
  void set_landmark_ids();
  void set_landmark_cost(int cost) { landmarks_cost = cost; }
 ...
 private:
  ...
  int needed_cost;
  int landmarks_cost;
  __gnu_cxx::hash_map<pair<int, int>, LandmarkNode *, hash_int_pair>
      simple_lms_to_nodes;
  __gnu_cxx::hash_map<pair<int, int>, LandmarkNode *, hash_int_pair>
      disj_lms_to_nodes;
