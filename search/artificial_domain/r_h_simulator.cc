...
void RHSimulator::run() {
  ...
  dump();
  // replacing the successor_generator, initial state, axiom_evaluator, and
  // goals.
  ...
  g_variable_domain.push_back(std::numeric_limits<int>::max());
  g_variable_domain.push_back(std::numeric_limits<int>::max());
  // cout << INT_MAX << endl;
...
static Simulator *_parse(OptionParser &parser) {
  ...
  parser.add_option<int>("branching_factor", DEFAULT_BF,
