...
ArtificialSuccessorGenerator::ArtificialSuccessorGenerator(RHSimulator *p)
    : params(p) {
  operators.push_back(new ArtificialOperator(-1));
  operators.push_back(new ArtificialOperator(0));
  operators.push_back(new ArtificialOperator(1));
}
void ArtificialSuccessorGenerator::generate_applicable_ops(
    const State &curr, std::vector<const Operator *> &ops) {
  // Curr is not used here, but we don't want to change the signature.
  // The following line suppresses the warnings.
  (void)curr;
