...
public:
	int effect; // determines how this operator changes the goal distance d
				// after applying the operator the goal distance d will be d + effect
				// where effect = 0, -1, or 1.
	...
	virtual void get_effects(const State& state, std::vector<std::pair<int, int> >& effects) const{
		ArtificialHeuristic* heuristic = (ArtificialHeuristic*) g_simulator->get_heuristic();
