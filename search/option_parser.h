...
class TokenParser<Simulator *> {
 public:
  static inline Simulator *parse(OptionParser &p);
};

template <>
class TokenParser<Postprocessor *> {
 public:
  static inline Postprocessor *parse(OptionParser &p);
};

template <>
class TokenParser<Analyzer *> {
 public:
...
class OptionParser {
 public:
  ...
  // will be parsed as T). Usually T=SearchEngine*, ScalarEvaluator* or
  // LandmarkGraph*
  template <class T>
  T start_parsing();

  // add option with no default value, optional help
  // call with mandatory = false to specify an optional parameter without
  // default value
  template <class T>
  ...
  template <class T>
  void add_option(std::string k, T def_val, std::string h = "");

  void add_enum_option(std::string k, std::vector<std::string> enumeration,
  ...
  template <class T>
  void add_list_option(std::string k, std::vector<T> def_val,
                       std::string h = "");

  void error(std::string msg);
  void warning(std::string msg);

  Options parse();  // parse is not the best name for this function. It just
                    // does some checks and returns the parsed options Parsing
                    // happens before that. Change?
...
template <class T>
void OptionParser::add_option(std::string k, std::string h, OptionFlags flags) {
  if (help_mode_) {
  ...
  if (arg->key.size() == 0) ++next_unparsed_argument;
}

template <class T>
void OptionParser::add_list_option(std::string k, std::vector<T> def_val,
                                   std::string h) {
  opts.set(k, def_val);
...
template <class T>
void OptionParser::add_option(std::string k, T def_val, std::string h) {
  opts.set(k, def_val);
  add_option<T>(k, h);
}
...
ScalarEvaluator *TokenParser<ScalarEvaluator *>::parse(OptionParser &p) {
  ParseTree::iterator pt = p.get_parse_tree()->begin();
  if (Predefinitions<Heuristic *>::instance()->contains(pt->value)) {
    return (ScalarEvaluator *)Predefinitions<Heuristic *>::instance()->get(
        pt->value);
  } else if (Registry<ScalarEvaluator *>::instance()->contains(pt->value)) {
...
Simulator *TokenParser<Simulator *>::parse(OptionParser &p) {
  ...
  if (Registry<Simulator *>::instance()->contains(pt->value)) {
    return Registry<Simulator *>::instance()->get(pt->value)(p);
  }
  p.error("simulator not found");
  return 0;
}

Postprocessor *TokenParser<Postprocessor *>::parse(OptionParser &p) {
  ParseTree::iterator pt = p.get_parse_tree()->begin();
  if (Registry<Postprocessor *>::instance()->contains(pt->value)) {
    return Registry<Postprocessor *>::instance()->get(pt->value)(p);
  }
  p.error("postprocessor not found");
  return 0;
}

Analyzer *TokenParser<Analyzer *>::parse(OptionParser &p) {
  ParseTree::iterator pt = p.get_parse_tree()->begin();
  if (Registry<Analyzer *>::instance()->contains(pt->value)) {
    return Registry<Analyzer *>::instance()->get(pt->value)(p);
  }
  p.error("analyzer not found");
  return 0;
}
...
std::vector<T> TokenParser<std::vector<T> >::parse(OptionParser &p) {
  ...
  if (pt->value.compare("list") != 0) {
    ...
    for (ParseTree::sibling_iterator pti =
         ...
         pti != end_of_roots_children(*p.get_parse_tree()); ++pti) {
