...
struct ParseNode {
  ...
  ParseNode(std::string val, std::string k = "") : value(val), key(k) {}
  std::string value;
  std::string key;

  friend std::ostream &operator<<(std::ostream &out, const ParseNode &pn) {
    if (pn.key.compare("") != 0) out << pn.key << " = ";
    out << pn.value;
    return out;
  }
};
...
struct ParseError {
  ...
  friend std::ostream &operator<<(std::ostream &out, const ParseError &pe) {
    out << "Parse Error: " << std::endl << pe.msg << " at: " << std::endl;
...
class Registry {
 public:
  ...
  Factory get(std::string k) { return registered[k]; }
...
class Predefinitions {
 public:
  ...
  T get(std::string k) { return predefined[k]; }
...
// There's something built in for this (typeid().name()), but the output is not
...
struct TypeNamer<LandmarkGraph *> {
  static std::string name() { return "landmarks graph"; }
};
...
template <>
struct TypeNamer<Analyzer *> {
  static std::string name() { return "analyzer"; }
};

template <>
struct TypeNamer<Postprocessor *> {
...
struct TypeNamer<SearchEngine *> {
  static std::string name() { return "search engine"; }
};

template <>
struct TypeNamer<ParseTree> {
  static std::string name() {
    return "parse tree (this just means the input is parsed at a later point. "
...
struct TypeNamer<Synergy *> {
  static std::string name() { return "synergy"; }
};

template <class Entry>
struct TypeNamer<OpenList<Entry> *> {
  static std::string name() { return "openlist"; }
};

template <class T>
struct TypeNamer<std::vector<T> > {
  static std::string name() { return "list of " + TypeNamer<T>::name(); }
};

// DefaultValueNamer is for printing default values.
// Maybe a better solution would be to implement "<<" for everything that's
// needed. Don't know.
...
template <class T>
tree<T> subtree(const tree<T> &tr, typename tree<T>::sibling_iterator ti) {
...
class Options {
 public:
  Options(bool hm = false) : help_mode(hm) {}

  void set_help_mode(bool hm) { help_mode = hm; }
  ...
  T get(std::string key) const {
    ...
    it = storage.find(key);
    if (it == storage.end()) {
      std::cout << "attempt to retrieve nonexisting object of name " << key
  ...
  int get_enum(std::string key) const { return get<int>(key); }
...
struct OptionFlags {
  OptionFlags(bool mand = true) : mandatory(mand) {}
  bool mandatory;
};
