...
void Aras::run(vector<const Operator*>& plan) {
...
void Aras::run() {
  if (g_input_plan_filename != "") {
    ...
    if (!inputfile) {
      cerr << "Error: cannot open the file \"" << g_input_plan_filename << "\""
           << endl;
      abort();
    }
...
void Aras::iterative_action_elimination(vector<const Operator*>& plan) {
  const int LENGTH_LIMIT = 10000;
  int cutoff = 1000;
  while (plan.size() > LENGTH_LIMIT) {
    int i = 0;
    State current_state = *g_initial_state;
    for (; i < plan.size() - cutoff; i++) {
      current_state.update(*plan[i]);
    }
    cutoff = min(LENGTH_LIMIT, int(cutoff * 2));
    action_elimination(current_state, plan, i);
  }
  action_elimination(*g_initial_state, plan, 0);
}

void Aras::action_elimination(State& initial_state,
                              vector<const Operator*>& plan, int index) {
  // cout << "index: " << index << endl;
  vector<bool> removed;
  removed.resize(plan.size(), false);
  int num_removed = 0;
  State current_state = initial_state;
  for (int i = index; i < plan.size(); ++i) {
    // if (i % 1000 == 0 )
    //	cout << i << endl;
  ...
  plan.erase(plan.begin() + current, plan.end());
}
...
vector<const Operator*> Aras::iterative_PNGS(vector<const Operator*>& plan) {
  ...
  for (int i = 0; i < num_iterations; ++i) {
    ...
    try {
      ...
      if (ae) iterative_action_elimination(plan);
    } catch (FullMemory e) {
      cout << "Memory Usage: " << mem << endl;
      cout << "Memory-limit is exceeded, estimated memory usage: "
           << (e.memory * OVERHEAD_FACTOR) / 1000 << " KB" << endl;
      cout << " Aras_Time: " << *timer << endl;
      if (current_cost == initial_cost) hibernate = true;
      reinit();
      delete timer;
      return plan;
    } catch (FinishTime e) {
      ...
      return plan;
       ...
       // HACK!!! regression expansion does not work with derived predicates
           // and leads to invalid plans. For now, if it happens we just switch
    off
           ...
           assert(backup_plan.size() == plan.size());
           cout << "Found an invalid plan. Switch off the recursive expansion "
    << endl; return plan;
    }*/
...
vector<const Operator*> Aras::PNGS(int start, int finish,
                                   NeigborhoodGraph& search_space,
                                   vector<const Operator*>& plan) {
...
int Aras::backward_blind_evaluation(State state) {
  if (state == *g_initial_state) return 0;
  return g_min_action_cost;
}

void Aras::build_progression_graph(int start, int finish,
                                   NeigborhoodGraph& search_space,
                                   vector<const Operator*>& plan) {
  ...
  for (int i = start; i <= finish; ++i) {
    ...
    if (i == start) node.open_initial(initial_h_value);
...
void Aras::progression_expand(NeigborhoodGraph& search_space,
                              Node& initial_node) {
  ...
  open_list.insert(make_pair(initial_node.get_f(), initial_node.get_h()),
                   initial_node.get_state_buffer());
  ...
  while (generated_states < steps_per_node && !open_list.empty()) {
    ...
    // search_space.memory_usage() << " close_list: " << close_list.capacity()
    // << endl;
    size_t mem_usage = open_list.memory_usage() + search_space.memory_usage() +
                       close_list.capacity() * sizeof(state_var_t*);
    if (memory_is_full(mem_usage)) throw FullMemory(mem_usage);
    if (exceed_time_limit()) throw FinishTime();
    ...
    for (int i = 0; i < applicable_ops.size(); i++) {
      ...
      if (succ_node.is_new()) {
        ...
        open_list.insert(make_pair(succ_node.get_f(), succ_h),
      ...
      if (succ_node.is_open()) {
        if (node.get_g() + op_cost < succ_node.get_g()) {
          ...
          open_list.insert(make_pair(succ_node.get_f(), succ_node.get_h()),
                           succ_node.get_state_buffer());
        }
      } else {
  ...
  for (int i = 0; i < close_list.size(); ++i) {
    ...
    Node node = search_space.get_node(state);
    node.make_permanent();
    if (test_goal(state)) goals.insert(state);
  }
...
void Aras::build_regression_graph(NeigborhoodGraph& search_space,
                                  vector<const Operator*>& plan) {
...
void Aras::regression_expand(NeigborhoodGraph& search_space,
                             Node& initial_node) {
  int generated_states = 0;
  ArasOpenList<state_var_t*> open_list;
  vector<state_var_t*> close_list;
  ...
  while (generated_states < steps_per_node && !open_list.empty()) {
    ...
    predecessor_generator->generate_leading_operators(node.get_state(),
                                                      leading_ops);
    ...
    for (int i = 0; i < leading_ops.size(); i++) {
      ...
      if (pred_node.is_new()) {
        ...
        open_list.insert(make_pair(pred_node.get_f(), pred_h),
                         pred_node.get_state_buffer());
      }

      if (pred_node.is_open()) {
        if (node.get_g() + op_cost < pred_node.get_g()) {
          pred_node.update_and_reopen(node.get_g(), op_cost);
          open_list.insert(make_pair(pred_node.get_f(), pred_node.get_h()),
                           pred_node.get_state_buffer());
        }
      } else {
...
vector<const Operator*> Aras::chain_backward(NeigborhoodGraph& state_space) {
  ...
  while (true) {
    // cout << "openlist: " << open_list.memory_usage() << "search_space: " <<
    // search_space.memory_usage() << " state_space: " <<
    // state_space.memory_usage() << endl;
    size_t mem_usage = search_space.memory_usage() +
                       state_space.memory_usage() + open_list.memory_usage();
    if (memory_is_full(mem_usage)) throw FullMemory(mem_usage);
    if (exceed_time_limit()) throw FinishTime();
    ...
    const vector<pair<state_var_t*, const Operator*> > parents =
        paragon.get_parents();
...
static Postprocessor* _parse(OptionParser& parser) {
  parser.add_option<int>("memory_limit", DEFAULT_MEM_LIMIT,
                         "memory limit for aras");
  parser.add_option<int>("time_limit", DEFAULT_TIME_LIMIT,
                         "time limit for aras");
  parser.add_option<bool>("reg_graph", DEFAULT_RG_GRAPH,
                          "build the regression graph");
  parser.add_option<bool>("ae", DEFAULT_AE, "use action elimination");
  parser.add_option<int>("num_iterations", DEFAULT_N_ITER,
                         "number of iterations");
  parser.add_option<int>("spn", DEFAULT_SPN,
                         "steps per node in the neighborhood graph");

  vector<string> cost_types;
  cost_types.push_back("NORMAL");
  cost_types.push_back("ONE");
  cost_types.push_back("PLUSONE");
  parser.add_enum_option("cost_type", cost_types, "PLUSONE",
