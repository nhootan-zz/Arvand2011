...
class NodeInfo {
 ...
 public:
  NodeInfo() : status(NEW), expand_status(NOT_EXPANDED), h(-1), g(-1) {}
};
...
class StateProxy {
 ...
 public:
  ...
  ~StateProxy() {
    if (!borrowed_buffer) delete[] state_data;
  }
  StateProxy() {
    state_data = 0;
...
Node::Node(state_var_t *state_buffer_, NodeInfo &info_,
           size_t *parents_num_byte_)
    : state_buffer(state_buffer_),
      info(info_),
      parents_num(parents_num_byte_) {}

State Node::get_state() const { return State(state_buffer); }

bool Node::is_goal() const {
  State s = State(state_buffer);
  for (int i = 0; i < g_goal.size(); i++)
    if (s[g_goal[i].first] != g_goal[i].second) return false;
  return true;
}
...
bool Node::is_new() const { return info.status == NodeInfo::NEW; }

bool Node::is_open() const { return info.status == NodeInfo::OPEN; }

bool Node::is_closed() const { return info.status == NodeInfo::CLOSED; }

bool Node::is_dead_end() const { return info.status == NodeInfo::DEAD_END; }

int Node::get_f() const { return info.g + info.h; }

int Node::get_g() const { return info.g; }

int Node::get_h() const { return info.h; }

int Node::get_parent_num() const { return info.parents.size(); }
...
void Node::add_parent(const Node &parent_Node, const Operator *parent_op) {
  ...
  // cout << "parents_num: " << (*parents_num) << " parents.size(): " <<
  // info.parents.size() << endl;
...
void Node::lazy_open(int h, const Node &parent_Node,
...
void Node::make_permanent() {
  if (info.status == NodeInfo::CLOSED) info.expand_status = NodeInfo::EXPANDED;
...
void Node::dump() {
  ...
  if (info.parents.size() > 0) {
    cout << " created by " << info.parents.begin()->second->get_name()
         << " from " << info.parents.begin()->first << endl;
  }
}

class NeigborhoodGraph::HashTable
    : public __gnu_cxx::hash_map<StateProxy, NodeInfo> {
...
NeigborhoodGraph::~NeigborhoodGraph() { delete nodes; }

int NeigborhoodGraph::size() const { return nodes->size(); }
size_t NeigborhoodGraph::memory_usage() const {
  size_t num_nodes = nodes->size();
  size_t size_state =
      sizeof(State) + sizeof(state_var_t) * g_variable_domain.size();
  size_t parents_memory =
      parents_num * (sizeof(pair<state_var_t *, const Operator *>) +
                     sizeof(int) + 3 * sizeof(int *));
  size_t hash_memory = ((nodes->bucket_count() + num_nodes) * sizeof(int *) +
                        sizeof(pair<StateProxy, NodeInfo>) * num_nodes);
...
void NeigborhoodGraph::trace_path(const State &goal_state,
  ...
  for (;;) {
    ...
    if (info.parents.size() == 0) break;

    const Operator *op = info.parents.begin()->second;
    path.push_back(op);
    current_state =
        StateProxy(const_cast<state_var_t *>(info.parents.begin()->first));
  }
}
...
void NeigborhoodGraph::dump() {
  ...
  for (HashTable::iterator iter = nodes->begin(); iter != nodes->end();
    ...
    if (iter->second.parents.size() > 0) {
      cout << " created by " << iter->second.parents.begin()->second->get_name()
           << " from " << iter->second.parents.begin()->first << endl;
    } else {
