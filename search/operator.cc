...
Prevail::Prevail(istream &in) { in >> var >> prev; }

PrePost::PrePost(istream &in) {
  int condCount;
  in >> condCount;
  for (int i = 0; i < condCount; i++) cond.push_back(Prevail(in));
  in >> var >> pre >> post;
}
...
Operator::Operator(istream &in, bool axiom) {
  ...
  if (!is_an_axiom) {
    ...
    int count;
    in >> count;
    for (int i = 0; i < count; i++) prevail.push_back(Prevail(in));
    in >> count;
    for (int i = 0; i < count; i++) pre_post.push_back(PrePost(in));
...
void Prevail::dump() const { cout << g_variable_name[var] << ": " << prev; }
...
void Operator::get_effects(const State &state,
  ...
  for (int i = 0; i < pre_post.size(); i++) {
    if (pre_post[i].does_fire(state)) {
      effects.push_back(make_pair(pre_post[i].var, pre_post[i].post));
    }
  }
}
