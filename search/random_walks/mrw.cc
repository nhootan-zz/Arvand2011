...
MRW::~MRW() { delete walker; }

void MRW::update_h_speed() {
  num_episodes++;
  if (num_nonwasted_walks != 0) {
    double current_speed =
        (initial_value - total_min) / double(num_nonwasted_walks);
    avg_speed = (current_speed - avg_speed) / double(num_episodes) + avg_speed;
    cout << "nonwasted walks: " << num_nonwasted_walks
         << " avg speed: " << avg_speed << endl;
  } else {
...
bool MRW::should_jump(int progress) {
  if (progress != 0) {
    if (local_stats.num_walks > params.nj) {
      ...
      // cout << "progress: " << progress << " ap: " << acceptable_progress <<
      // endl;
      return true;
    } else
      return false;
...
int MRW::search_step() {
  ...
  while (true) {
    ...
    params.change_rl();
    walker->random_walk(current_state, preferred_ops, total_min, current_cost,
                        bound, params);
    // current_state.dump();
...
void MRW::update_acceptable_progress(int progress) {
  if (params.acceptable_progress == false) return;
  if (acceptable_progress == -1) {
    // first step
    acceptable_progress = progress;
  } else
    acceptable_progress =
        (1 - params.alpha) * acceptable_progress + params.alpha * progress;

  if (acceptable_progress < 0.001) acceptable_progress = 0;
}
...
void MRW::report_progress() {
  ...
  cout << "h_min: " << total_min << " #walks: " << global_stats.num_walks
       << " #evaluations: " << global_stats.num_evaluated
       << " cost: " << current_cost << endl;
  // cout << "#g_evaluated: " << global_stats.num_evaluated << " #g_generated: "
  // << global_stats.num_generated << " g_stats"<< endl;
}
...
static SearchEngine *_parse(OptionParser &parser) {
  ...
  parser.add_option<int>("tg", DEFAULT_TG,
                         "the global restarting threshold; meaningless if "
                         "deterministic_gr = true");
  parser.add_option<bool>("adaptive_lr", DEFAULT_AP_RL,
                          "adaptive local restarting");
  parser.add_option<bool>("adaptive_gr", DEFAULT_DNRG,
                          "adaptive global restarting");
  parser.add_option<bool>("deterministic_gr", DEFAULT_DTRG,
                          "true = use a fix threshold (tg) for global restart; "
                          "false = use a global restarting rate");
  ...
  parser.add_option<bool>("acceptable_progress", DEFAULT_AP,
                          "use acceptable progress for jumping");
  parser.add_option<double>("alpha", DEFUALT_ALPHA,
                            "used to compute acceptable progress: meaningless "
                            "if acceptable_progress = false");
  // parser.add_option<int>("eval_limit", DEFAULT_EVAL_LIMIT, "eval limit");
  parser.add_option<double>("mda_t", DEFUALT_MDA_T,
                            "MDA temperature: only used for bias = MDA");
  parser.add_option<double>("epsilon", DEFAULT_EPSILON,
                            "Epsilon used for adaptive_lr");
  parser.add_option<double>(
      "mha_t", DEFUALT_MHA_T,
      "MHA temperature: only used for bias = MHA or AGRESSIVE_MHA");
  parser.add_option<double>("mha_w", DEFUALT_MHA_W,
                            "the weight of current preferred ops in MHA"
                            ": only used for bias = MHA or AGRESSIVE_MHA");
  parser.add_option<bool>("sample_unused", DEFAULT_S_UNUSED,
                          "first sample unused actions");
  ...
  parser.add_enum_option("bias", bias_types, "MHA", "RW bias");
