		...
		if(r < epsilon || num_samples == 0){
			...
			for (i = 0; i < num.size(); ++i) {
				if(num[i] != 0 ){
					...
					// now we choose the setting with highest Q =value/cost: tie break by cost.
					// this favors shorter walks at the beginning.
					assert(cost[i] != 0);
					double avg_cost = cost[i]/double(num[i]);
					double avg_value = value[i]/double(num[i]);
					double Q = avg_value/avg_cost;
					//double Q = avg_value;
					//cout << rl_candidates[i] << " Q: " << Q << " cost: " << avg_cost << endl;
					if(Q > best_q){
						best_q = Q;
						best_cost = avg_cost;
						best_index = i;
...
void MRW_Parameters::dump(){
	...
	if(bias == UNIFORM)
		...
		cout << "MHA" << " T: " << mha_temperature << " w: " << mha_w << endl;
	} else if(bias == AGRESSIVE_MHA){
		cout << "AGRESSIVE_MHA" << " T: " << mha_temperature << " w: " << mha_w << endl;
	} else if(bias == MDA){
		cout << "MDA" << " T: " << mda_temperature << endl;
	}
	...
	if(deterministic_gr)
		cout << "deterministic_gr: True" << " tg: " << tg << endl;
	else
		cout << "deterministic_gr: False" << " rg: " << rg << endl;
