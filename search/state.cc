...
State::State(const Operator &op, const State &successor) {
  ...
  for (int i = 0; i < op.get_pre_post().size(); i++) {
    ...
    if (pre_post.pre != -1) vars[pre_post.var] = pre_post.pre;
  }
...
void State::_deallocate() {
  if (!borrowed_buffer) delete[] vars;
}
...
void State::_copy_buffer_from_state(const State &state) {
  ...
  for (int i = 0; i < g_variable_domain.size(); i++) vars[i] = state.vars[i];
}

State &State::operator=(const State &other) {
  if (this != &other) {
    if (borrowed_buffer) _allocate();
...
void State::dump() const {
  ...
  // if state_var_t == char.
  for (int i = 0; i < g_variable_domain.size(); i++)
    cout << "  " << g_variable_name[i] << ": " << static_cast<int>(vars[i])
         << endl;
}
...
bool State::operator<(const State &other) const {
  int size = g_variable_domain.size();
  return ::lexicographical_compare(vars, vars + size, other.vars,
                                   other.vars + size);
}
