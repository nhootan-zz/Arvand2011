...
CGHeuristic::~CGHeuristic() {
  for (int i = 0; i < prio_queues.size(); ++i) delete prio_queues[i];
}

bool CGHeuristic::dead_ends_are_reliable() const { return false; }
...
int CGHeuristic::get_transition_cost(const State &state,
                                     ...
                                     int goal_val) {
  if (start_val == goal_val) return 0;
  ...
  if (start->distances.empty()) {
    ...
    while (!prio_queue.empty()) {
      ...
      if (start->distances[source->value] < source_distance) continue;
  ...
  if (use_the_cache) {
    for (int val = 0; val < start->distances.size(); val++) {
      ...
      cache->store_helpful_transition(var_no, state, start_val, val, helpful);
    }
  }
