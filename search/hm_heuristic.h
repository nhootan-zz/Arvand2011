 ...
 protected:
  ...
  int get_operator_pre_value(const Operator &op, int var) {
    for (int i = 0; i < op.get_prevail().size(); i++) {
      if (op.get_prevail()[i].var == var) return op.get_prevail()[i].prev;
    }

    for (int i = 0; i < op.get_pre_post().size(); i++)
      if (op.get_pre_post()[i].var == var) return op.get_pre_post()[i].pre;
  ...
  void get_operator_pre(const Operator &op, tuple &t) {
    ...
    for (int i = 0; i < op.get_pre_post().size(); i++)
      if (op.get_pre_post()[i].pre >= 0)
        t.push_back(
            make_pair(op.get_pre_post()[i].var, op.get_pre_post()[i].pre));
  ...
  void get_operator_eff(const Operator &op, tuple &t) {
    for (int i = 0; i < op.get_pre_post().size(); i++)
      t.push_back(
          make_pair(op.get_pre_post()[i].var, op.get_pre_post()[i].post));
  ...
  void generate_all_partial_tuple(tuple &base_tuple, tuple &t, int index,
                                  int sz, vector<tuple> &res);
