...
void LazySearch::set_pref_operator_heuristics(vector<Heuristic *> &heur) {
...
void LazySearch::get_successor_operators(vector<const Operator *> &ops) {
  ...
  if (succ_mode == pref_first) {
    ...
    for (int i = 0; i < all_operators.size(); i++)
      if (!all_operators[i]->is_marked()) ops.push_back(all_operators[i]);
  } else {
    for (int i = 0; i < preferred_operators.size(); i++)
      if (!preferred_operators[i]->is_marked()) preferred_operators[i]->mark();
    ops.swap(all_operators);
    if (succ_mode == shuffled) random_shuffle(ops.begin(), ops.end());
  }
}
...
void LazySearch::generate_successors() {
  ...
  for (int i = 0; i < operators.size(); i++) {
    ...
    if (is_preferred) operators[i]->unmark();
    if (new_real_g < bound) {
  ...
  if (node.is_new() || reopen) {
    ...
    for (int i = 0; i < heuristics.size(); i++) {
      if (current_operator != NULL) {
        heuristics[i]->reach_state(parent_node.get_state(), *current_operator,
                                   perm_state);
      }
    ...
    if (!open_list->is_dead_end()) {
      ...
      node.close();
      if (check_goal_and_set_plan(current_state)) return SOLVED;
...
void LazySearch::statistics() const { search_progress.print_statistics(); }
  ...
  if (!parser.dry_run()) {
    ...
    for (int i = 0; i < evals.size(); i++) {
      ...
      if (opts.get<int>("w") == 1) {
        ...
        WeightedEvaluator *w =
            new WeightedEvaluator(evals[i], opts.get<int>("w"));
...
static Plugin<SearchEngine> _plugin_weighted_astar("lazy_wastar",
                                                   _parse_weighted_astar);
