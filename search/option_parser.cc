...
ParseError::ParseError(string m, ParseTree pt) : msg(m), parse_tree(pt) {}

HelpElement::HelpElement(string k, string h, string t_n)
    : kwd(k), help(h), type_name(t_n) {}
...
static void get_help_templ(const ParseTree &pt) {
  if (Registry<T>::instance()->contains(pt.begin()->value)) {
    cout << pt.begin()->value << " is a " << TypeNamer<T>::name() << endl
         << "Usage: " << endl;
...
static void predefine_heuristic(std::string s, bool dry_run) {
  ...
  if (definees.size() == 1) {  // normal predefinition
    ...
    if (!dry_run) {
      std::vector<Heuristic *> heur = op.start_parsing<Synergy *>()->heuristics;
      for (size_t i(0); i != definees.size(); ++i) {
        Predefinitions<Heuristic *>::instance()->predefine(definees[i],
                                                           heur[i]);
      }
    } else {
      for (size_t i(0); i != definees.size(); ++i) {
        Predefinitions<Heuristic *>::instance()->predefine(definees[i], 0);
      }
    }
  } else {
...
SearchEngine *OptionParser::parse_cmd_line(int argc, const char **argv,
                                           bool dry_run) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    // Begin
      ...
      ++i;
      OptionParser p(argv[i], dry_run);
      g_simulator = p.start_parsing<Simulator *>();
    } else if (arg.compare("--analyzer") == 0) {
      ++i;
      OptionParser p(argv[i], dry_run);
      g_analyzer = p.start_parsing<Analyzer *>();
    } else if (arg.compare("--postprocessor") == 0) {
      ++i;
      OptionParser p(argv[i], dry_run);
    ...
    } else if (arg.compare("--input-plan-file") == 0) {
      ++i;
      g_input_plan_filename = argv[i];
    } else {
...
string OptionParser::usage(string progname) {
  string usage =
      ...
      "--input-plan-file FILENAME\n"
      "    Plan will be used as an input for postprocessor\n\n"
...
static ParseTree generate_parse_tree(string config) {
  ...
  for (size_t i(0); i != config.size(); ++i) {
    ...
    switch (next) {
      ...
      case '(':
        ...
        break;
      case ')':
        if (cur_node == top) throw ParseError("missing (", *cur_node);
  ...
  if (next != ')')
    throw ParseError("expected ) at end of configuration after " + buffer,
                     *cur_node);
...
void OptionParser::add_enum_option(string k, vector<string> enumeration,
  ...
  if (!flags.mandatory && !opts.contains(k)) return;
  ...
  if (!(str_stream >> x).fail()) {
    ...
    //...otherwise try to map the string to its position in the enumeration
    //vector
...
Options OptionParser::parse() {
  ...
  for (ParseTree::sibling_iterator pti = first_child_of_root(parse_tree);
    ...
    if (pti->key.compare("") != 0) {
      ...
      if (!valid_key) {
        error("invalid keyword " + pti->key + " for " +
...
bool OptionParser::dry_run() const { return dry_run_; }

bool OptionParser::help_mode() const { return help_mode_; }

void OptionParser::set_parse_tree(const ParseTree &pt) { parse_tree = pt; }

ParseTree *OptionParser::get_parse_tree() { return &parse_tree; }
