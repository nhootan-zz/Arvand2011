...
bool ParetoOpenList<Entry>::is_nondominated(const KeyType &vec,
  ...
  for (typename KeySet::iterator it = domination_candidates.begin();
    ...
    if (dominates(*it, vec)) return false;
  }
  return true;
}
...
void ParetoOpenList<Entry>::remove_key(const KeyType key) {
  ...
  buckets.erase(key);
  KeySet candidates;
  for (typename BucketMap::iterator it = buckets.begin(); it != buckets.end();
       ++it)
    ...
    if (nondominated.find(it->first) == nondominated.end() &&
        dominates(key, it->first) && is_nondominated(it->first, nondominated))
      candidates.insert(it->first);
  for (typename KeySet::iterator it = candidates.begin();
       it != candidates.end(); ++it)
    if (is_nondominated(*it, candidates)) nondominated.insert(*it);
}

template <class Entry>
ParetoOpenList<Entry>::ParetoOpenList(
    const std::vector<ScalarEvaluator *> &evals, bool preferred_only,
    bool state_uniform_selection_)
    : OpenList<Entry>(preferred_only),
      state_uniform_selection(state_uniform_selection_),
      evaluators(evals) {
...
int ParetoOpenList<Entry>::insert(const Entry &entry) {
  if (OpenList<Entry>::only_preferred && !last_preferred) return 0;
...
Entry ParetoOpenList<Entry>::remove_min(vector<int> *key) {
  ...
  for (typename KeySet::iterator it = nondominated.begin();
    ...
    if (state_uniform_selection)
      ...
      numerator = 1;
    seen += numerator;
    if ((rand() % seen) < numerator) selected = it;
  }
  if (key) {
    assert(key->empty());
    *key = *selected;
