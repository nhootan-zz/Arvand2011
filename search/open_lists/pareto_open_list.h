...
class ParetoOpenList : public OpenList<Entry> {
  ...
  typedef
      typename __gnu_cxx::hash_map<const KeyType, Bucket,
                                   __gnu_cxx::hash<const std::vector<int> > >
          BucketMap;
  ...
  bool is_nondominated(const KeyType &vec, KeySet &domination_candidates);
